...
import type { AddKeyInput, ApiKeyInput, ProviderInput } from "../shared/types";
...
async function handleApi(
  ...
  if (method === "GET" && keyProxyUrlMatch) {
    ...
    if (!provider || !apiKey?.proxyBaseUrl) throw notFound("API key not found", "api_key_not_found");
    sendJson(res, 200, { url: apiKey.proxyBaseUrl });
    return;
  }
  ...
  if (method === "GET" && proxyUrlMatch) {
    ...
    if (!firstKey?.proxyBaseUrl) throw notFound("API key not found", "api_key_not_found");
    sendJson(res, 200, { url: firstKey.proxyBaseUrl });
    return;
  }
