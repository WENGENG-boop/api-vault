...
test("builds SDK-friendly proxy base URL for OpenAI-compatible providers", () => {
  assert.equal(
    buildProxyBaseUrl(12345, "provider-1", "key-1", "https://vendor.example", "openai-compatible"),
    "http://127.0.0.1:12345/proxy/provider-1/key-1/v1"
  );
  assert.equal(
    buildProxyBaseUrl(12345, "provider-1", "key-1", "https://vendor.example/openai", "openai-compatible"),
    "http://127.0.0.1:12345/proxy/provider-1/key-1/openai/v1"
  );
  assert.equal(
  ...
  assert.equal(
    ...
    "http://127.0.0.1:12345/proxy/provider-1/key-1"
  );
});
