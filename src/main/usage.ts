...
function findModelString(value: unknown, depth = 0): string | undefined {
  if (!value || typeof value !== "object" || depth > 4) return undefined;
  const object = value as Record<string, unknown>;
  const direct = object.model ?? object.model_id ?? object.modelName ?? object.model_name;
  if (typeof direct === "string" && direct.trim()) return direct.trim();

  for (const key of ["body", "request", "parameters", "params", "payload", "data", "options"]) {
    const nested = findModelString(object[key], depth + 1);
  ...
  for (const item of Object.values(object)) {
    if (Array.isArray(item)) {
      for (const child of item) {
        const nested = findModelString(child, depth + 1);
        if (nested) return nested;
      }
    }
  }
